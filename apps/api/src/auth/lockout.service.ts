 ...
 *
 * Replaces a process-local `Map` keyed on phone alone. That design had two
 * defects that only show up in production:
 *
 ...
 *
 *  2. **It was a denial-of-service weapon.** The phone number is the login
 *     identifier — it is not a secret. Five deliberate wrong passwords locked
 ...
 *
 *  - **Per (phone, IP)** — tight and hard. Five failures locks that IP out of
 *    that account. This is the real brute-force control, and an attacker can
 *    only ever lock *themselves* out of it.
 *  - **Per phone, globally** — deliberately loose. It takes far more failures,
 *    across many IPs, before the account itself locks. Reaching it means a
 *    genuine distributed attack, where locking is the lesser harm. Driving it
 *    as a DoS costs an attacker many IPs *and* runs into the per-IP request
 *    limiter first.
 *
 * There is no FK to User: attempts against phone numbers that do not exist must
 * be counted too, or the table becomes an account-enumeration oracle.
 */

/** Per (phone, IP). Low, because a legitimate user rarely fails five times. */
const PAIR_MAX_FAILURES = 5
const PAIR_WINDOW_MS = 15 * 60 * 1000

/** Per phone, all IPs. High, because this one is reachable by third parties. */
...
export class LockoutService {
   ...
   */
  async assertNotLocked(phone: string, ipKey: string): Promise<void> {
    const retryAfterMs = await this.lockedUntil(phone, ipKey)
  ...
  private async lockedUntil(phone: string, ipKey: string): Promise<number | null> {
    const now = this.now()

    const [pairOldest, globalOldest] = await Promise.all([
      this.oldestWithinWindow(phone, PAIR_WINDOW_MS, PAIR_MAX_FAILURES, ipKey),
      this.oldestWithinWindow(phone, GLOBAL_WINDOW_MS, GLOBAL_MAX_FAILURES),
    ])
  ...
  private async oldestWithinWindow(
    ...
    const where = { phone, attemptedAt: { gte: since }, ...(ipKey ? { ipKey } : {}) }
  ...
  async recordFailure(phone: string, ipKey: string, kind: AttemptKind = 'login'): Promise<void> {
    ...
    await this.prisma.loginAttempt.create({
      data: { phone, ipKey, kind, attemptedAt: new Date(this.now()) },
    })
    ...
    // this phone so the write stays small and indexed.
    await this.prisma.loginAttempt.deleteMany({
      where: { phone, attemptedAt: { lt: new Date(this.now() - RETENTION_MS) } },
    })
  }

  /**
   * Clears a phone's failures.
   *
  ...
  async clear(phone: string): Promise<void> {
    await this.prisma.loginAttempt.deleteMany({ where: { phone } })
  }
}
